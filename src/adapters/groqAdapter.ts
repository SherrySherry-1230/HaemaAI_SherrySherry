...
/**
 * Groq 어댑터 구현
 * Groq SDK를 사용하여 빠른 추론 모델(Llama 시리즈 등)을 호출한다.
 */
...
export class GroqAdapter implements AIAdapter {
  ...
  constructor(config: AIAdapterConfig) {
    ...
    if (!config.apiKey) {
      throw new Error('GroqAdapter requires apiKey in config');
    }
...
대화 내용을 분석하여 쩜(JJum)(기억 단위)를 추출하세요.
...
요약을 작성하세요.`;
  ...
  async judgeMergeCandidate(a: JJum, b: JJum, hint?: string): Promise<MergeJudgement> {
    ...
    const system = `당신은 두 쩜(JJum)가 같은 대상인지 판정하는 AI입니다.
