...
/**
 * 점(JJum) 스키마 v3 — 단일 기준: docs/SCHEMA.md (2026-09-12 확정)
 * 점 = AI와 유저가 공유하는 생각·기억의 최소 단위. 점 사이 연관 = 선(Seon), 점의 표식 = H-tag(tags).
 *
...
/**
 * 점 — 논리 경로 jjums/{JJumId} (물리 경로는 어댑터가 정한다).
 * 파일 어댑터 실제 경로: local-server/haema/{ownerId}/{canonicalName}.jj
 */
export interface JJum {
  ...
  /** 별칭 — ["핑크", "혜정이"]. 어느 이름으로 언급돼도 같은 점 히트 */
  aliases: string[];
  ...
  // ═══ 내용 ═══
  /** 점 한 줄 요약 — "월 1~2회 만나는 친한 친구" (배치가 생성·갱신) */
  summary: string;
  /** 사실 조각들 */
  facts: JjumFact[];
  /** 시간축 사건 */
