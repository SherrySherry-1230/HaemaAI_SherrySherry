...
/**
 * 고민 쩜 정책 (2단계 확정 2026-09-06)
 *
 * - type "고민" 은 정식 종류. 상태는 meta 가 아니라 JJ-tag 로 — "미해결" / "해결".
...
export function isUnresolvedConcern(cell: Pick<JJum, 'type' | 'jjtags'>): boolean {
  return isConcern(cell) && cell.jjtags.some((t) => norm(t) === TAG_UNRESOLVED);
}

/**
 * 고민을 해결 상태로 바꾼다 — 미해결 태그를 떼고 해결 태그를 달며, 해결 시점을 사건으로 남긴다.
 * 쩜을 제자리에서 고치고 같은 객체를 돌려준다 (저장은 호출자가).
 */
export function resolveConcern(cell: JJum, now: JJumTimestamp, note?: string): JJum {
  cell.jjtags = cell.jjtags.filter((t) => norm(t) !== TAG_UNRESOLVED);
  if (!cell.jjtags.some((t) => norm(t) === TAG_RESOLVED)) cell.jjtags.push(TAG_RESOLVED);
  cell.events.push({ date: now, summary: note ? `해결: ${note}` : '해결됨', refJJumIds: [] });
  cell.editHistory.push({ date: now, action: 'resolve', field: 'jjtags', by: 'user' });
  return cell;
}
