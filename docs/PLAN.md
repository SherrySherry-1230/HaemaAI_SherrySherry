...
> 작성: 2026-09-04 (쉐리쉐리). 기준: `SCHEMA.md`(쩜(jjum) 스키마, 확정) · `SCENARIOS.md`(테스트 정답지) ·
...
- 하는 일: ① 대화에서 쩜 추출 ② summary·관계 요약 생성 ③ 병합 후보 판정 ④ 회상 후보 점수화
...
- [x] `node:test` 계약 스위트 통과: jjumName **또는 어느 별칭으로도** `findByName` 히트 /
...
콘솔로 만든 쩜 .jj를 복사해 피피/푸푸 프롬프트에 수동 주입하면, 회상 발화 톤을
...
| ③ | **이야깃거리** | 대화가 끊길 때 던질 화제 — 오래 언급 안 된 쩜 · 다가오는 사건 · 팔로업 거리 |
...
- 종류: 팔로업(답 비어 있는 쩜) / 다가오는 것(날짜 임박) / 오래된 긍정 쩜 / 패턴(반복 주기 예정 시점)
...
- [~] **`docs/SCENARIOS.md` 재현: 시나리오 01(핑크 — 생성·이어묻기·회상) · 02(태그 단서 회상) · 03(부정 저장·편들기 가이드·위로 단서) · 킬러 데모(비 오는 날의 카페)** — 결정론 파트는 2-a 테스트로 재현됨(`tests/recall.test.ts` SCENARIOS 절). 쩜 자동 추출·summary 생성·mode 정교화(편들기 판정)는 2-b

**마이풉에서 테스트할 수 있는 것:**
검증 목표 시나리오 그 자체 — "어제도 핑크랑 술 먹고 놀았어"를 기록해 둔 뒤 "이번 주말 뭐 하지"라고 하면
핑크 쩜 + 선이 회상 후보로 주입되어 **"이번 주말엔 핑크랑 뭐 안 해?"** 가 재현되는가.
...
**마이풉에서 테스트할 수 있는 것:**
"핑크가 [박**]잖아" → 피피가 자연스럽게 확인 발화("잠깐, 핑크가 [박**]야?! 합쳐서 기억할게~")
...
**범위:**
- 쩜 상한 정책 — **Haema는 숫자를 모른다**, 호스트가 옵션으로 넘긴다 (호스트 기본 예시: **무료 80 / 프리미엄 500**, 2026-09-06 확정)
...
- 승계 유틸: 게스트(LocalStorage) 쩜 → 회원(Firestore) 업로드 병합 (마이풉 확정 정책:
  게스트=클라이언트 저장 / 회원=동기화)

**완료 기준:**
- [ ] 계약 스위트가 LocalStorage/Firestore 양쪽에서 통과 (어댑터 교체 구조 실증)
- [ ] 승계 라운드트립: 게스트 쩜 전부가 회원 계정으로 이전, 중복 없이
